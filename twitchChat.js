...
ComfyJS.onChat = (user, message, flags, self, extra) => {
    ...
    if (flags.subscriber) {
        chatHtml += '<img src="' + badgeData[extra.userBadges.subscriber].image_url_1x + '" alt="" class="twitch badge_broadcaster badge">';
    }
    ...
    if (extra.messageEmotes === null) {
        ...
        Object.entries(extra.messageEmotes).forEach(([id, positions]) => {
            ...
            stringReplacements.push({
                ...
                replacement: `<img class="emote" src="https://static-cdn.jtvnw.net/emoticons/v1/${id}/1.0">`,
            });
        });
                        ...
                        success: function (result) {
                            ...
                            for (let i = 0; i < result.data[0].versions.length; i++) {
                                badgeData[result.data[0].versions[i].id] = {
                                    ...
                                    'image_url_3x': result.data[0].versions[i].image_url_3x
                                }
                            }
                        },
